...
remark: 值为引用数据id，多个id间以英文逗号分隔，远程对象的 ref_delete_policy 会被忽略，ref_delete_policy只能为no_action/restrict
required: ref_id （引用对象id）, ref_delete_policy （引用对象数据被删除时是否阻止）
optional: default_value, formula, query_filter, ref_list_label （引用对象相关列表显示的标签）
rule: ref_delete_policy in (''restrict'', ''set_null'')
...
comment on column hymn.core_biz_object_field.standard_type is '标准类型 自定义字段不能设置该值，用于处理模块对象和标准对象的特殊字段的类型，预定义字段该值可能为null ;; optional_value:[create_by_id(创建人id), modify_by_id(修改人id),  create_date(创建时间), modify_date(修改时间), org_id(组织id), lock_state(锁定状态), name(名称), type_id(业务类型), owner_id(所有人id)]';
