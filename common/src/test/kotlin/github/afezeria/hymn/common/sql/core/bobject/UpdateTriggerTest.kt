...
class UpdateTriggerTest : BaseDbTest() {
    ...
    fun `cannot modify inactive object`() {
        ...
        adminConn.use {
            ...
            val up = it.execute(
                "update hymn.core_biz_object set name='bbbcd' where id =? returning *",
                id
            )[0]
    ...
    fun `cannot modify other properties when a object is enabled`() {
        ...
        adminConn.use {
            var c = it.execute(
                "update hymn.core_biz_object set active=false ,name = 'bbcd' where id =? returning *",
                id
            )[0]
            c["name"] shouldBe name
            c["active"] shouldBe false
            c = it.execute(
                "update hymn.core_biz_object set name='bbbcd',active=true where id =? returning *",
                id
            )[0]
            c["name"] shouldBe name
    ...
    fun `cannot modify source table`() {
        ...
        adminConn.use {
            val e = shouldThrow<PSQLException> {
                it.execute(
                    "update hymn.core_biz_object set source_table = 'core_account' where id = ?",
                    id
                )
            }
    ...
    fun `object cannot be inactive when it is referenced`() {
        ...
        adminConn.use {
            ...
            val e = shouldThrow<PSQLException> {
                it.execute("update hymn.core_biz_object set active=false where id = ?", o2Id)
            }
