...
class CoreCustomPages(alias: String? = null) :
    ...
    override fun doCreateEntity(row: QueryRowSet, withReferences: Boolean) = CustomPage(
        ...
        it.createById = requireNotNull(row[this.createById]) { "field CustomPage.createById should not be null" }
        it.createBy = requireNotNull(row[this.createBy]) { "field CustomPage.createBy should not be null" }
        it.modifyById = requireNotNull(row[this.modifyById]) { "field CustomPage.modifyById should not be null" }
        it.modifyBy = requireNotNull(row[this.modifyBy]) { "field CustomPage.modifyBy should not be null" }
        it.createDate = requireNotNull(row[this.createDate]) { "field CustomPage.createDate should not be null" }
        it.modifyDate = requireNotNull(row[this.modifyDate]) { "field CustomPage.modifyDate should not be null" }
    }
}
