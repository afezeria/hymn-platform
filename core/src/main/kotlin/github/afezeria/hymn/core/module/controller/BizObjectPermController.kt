...
class BizObjectPermController {
    ...
    fun findAll(
        ...
        val list = bizObjectPermService.pageFind(pageSize, pageNum)
        return list
    }
    ...
    @GetMapping("/{id}")
    fun findById(@PathVariable("id") id: String): BizObjectPerm {
        val entity = bizObjectPermService.findById(id)
            ?: throw ResourceNotFoundException("对象权限".msgById(id))
        return entity
    }
    ...
    @PostMapping
    fun create(@RequestBody dto: BizObjectPermDto): String {
    ...
    fun update(
        ...
        @RequestBody dto: BizObjectPermDto
    ): Int {
