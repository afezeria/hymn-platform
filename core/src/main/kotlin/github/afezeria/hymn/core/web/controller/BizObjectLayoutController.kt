 ...
 */
@RestController
@RequestMapping("/module/core/api/biz-object-layout")
@Api(tags = ["业务对象详情页面布局接口"])
...
(value = "查询全部数据", notes = "")
    @GetMapping
    fun findAll(): ApiResp<List<BizObjectLayout>> {
        val all = bizObjectLayoutService.findAll()
...
(value = "根据id查询", notes = "无参数时查询全部数据")
    @GetMapping("/{id}")
    fun findById(@PathVariable("id") id: String): ApiResp<BizObjectLayout> {
...
(value = "新建", notes = "")
    @PostMapping
    fun create(@RequestBody dto: BizObjectLayoutDto): ApiResp<String> {
...
(value = "更新", notes = "")
    ...
    fun update(@PathVariable("id") id: String,
               @RequestBody dto: BizObjectLayoutDto): ApiResp<Int> {
